...
drop db password guestbook-db --set-secret guestbook:PGPASSWORD   # rotate + store directly; never printed
...
`drop db password guestbook-db --set-secret guestbook:PGPASSWORD` → `drop start guestbook`.
