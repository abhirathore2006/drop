...
/** One shared README used (lightly specialized) by both seed templates. */
const readme = (name, image) =>
  `# ${name}

A golden-path ${name} template: a container app + a managed Postgres database, wired together.

## What it creates
- a **database** (\`db\`) — managed Postgres with a 1Gi volume by default
- an **app** (\`web\`) — the \`${image}\` image, bound to the database via \`uses\`
...
/** Build a template publish payload for an example (image-pinned; adds the two demo variables). */
function templateFor({ slug, name, image, cpu, memory }) {
  return {
    slug,
    name,
    description: `${name}: a container app + managed Postgres, wired together`,
    visibility: "public",
    variables: [
...
const SEEDS = [
  ...
  templateFor({ slug: "notes", name: "notes", image: "notes-next:1", cpu: "500m", memory: "512Mi" }),
];
