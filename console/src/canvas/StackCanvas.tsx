...
type StackNodeData = { node: GraphNode; pending?: GraphPlanStep["action"] };
...
function StackNode({ data }: NodeProps) {
  const { node, pending } = data as StackNodeData;
  return (
    <>
...
export default function StackCanvas({ graph }: { graph: StackGraph }) {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const nodes: Node[] = graph.nodes.map((n) => ({
      ...
      data: { node: n, pending: pending[n.key] } satisfies StackNodeData,
